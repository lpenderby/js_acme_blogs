...
const createSelectOptions = async () => {
    const userArray = await fetch("https://jsonplaceholder.typicode.com/users");
    const jsonUsers = await userArray.json();
    let optionArray = [];
    jsonUsers.forEach(user => {
        let option = document.createElement("option");
        option.value = user.id;
        option.textContent = user.name;
        optionArray.push(user);
    });
    console.log()
    return optionArray;
...
function createElemWithText(element = "p", text = "", name) {
    ...
    if (name) newElem.classList.add(name);
    return newElem;
}
...
//COME BACK WHEN TOGGLE COMMENTS FUNCTION IS DONE
function addButtonListeners () {
    const mainElement = document.querySelector("main");
    const buttons = mainElement.querySelectorAll("button");
    if (!buttons) return;
    buttons.forEach(button => {
        let postId = Number(button.dataset.postId);
...
async function getUserPosts(userID) {
    ...
    try {
        ...
        const filteredPosts = jsonPosts.filter(post => {
...
async function getUser(userID) {
    ...
    try {
        const users = await fetch("https://jsonplaceholder.typicode.com/users");
        const jsonUsers = await users.json();
        const filteredUser = jsonUsers.find(user => {
...
async function getPostComments(postID) {
    ...
    try {
        ...
        const filteredComments = jsonComments.filter(comment => {
...
function toggleComments (event, postId) {
    ...
    const toggleCommentsArray = [section, button];
    return toggleCommentsArray;
}
...
async function refreshPosts(posts) {
    ...
    const main = deleteChildElements(getMain);
    const fragment = await displayPosts(posts);
    const addButtons = addButtonListeners();
    let refreshPostsArray = [removeButtons, main, fragment, addButtons];
...
async function selectMenuChangeEventHandler(event) {
    ...
    if (typeof event.target === 'undefined') {
        event.target = document.querySelector("#selectMenu");
    }
    ...
    const selectMenuChangeEventHandlerArray = [userId, posts, refreshPostsArray];
    return selectMenuChangeEventHandlerArray;
}
...
async function initPage() {
    ...
    let initPageArray = [users, select];
    return initPageArray;
}
